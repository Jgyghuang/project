...
class garch(object):
    ...
    def __init__(self, dayreturn, daysigma, p, q):
        self.re = dayreturn
        t = sm.tsa.stattools.adfuller(self.re)
        if t[1] < 0.05:
            self.dayreturn = dayreturn
        else:
            for i in range(1, len(self.re)):
                dayreturn[i] = dayreturn[i] - dayreturn[i - 1]
            self.dayreturn = dayreturn

        # value of pacf
        pacf = statsmodels.tsa.stattools.pacf(self.dayreturn, nlags=19)
        ...
        if p_arch < 0.05:
            print('p value = ',p_arch)
            print('Pass the ARCH effect test.')
        else:
            print('p value = ',p_arch)
            print('Fail to pass the ARCH effect test. The original data cannot be used in our ARCH model.')
    ...
    # p value of ADF Test,
    # ? what if fail? test for stationary
    def adf(self):
        t = sm.tsa.stattools.adfuller(self.re)
        if t[1] < 0.05:
            return print('p value = ',t[1],', pass the stationary test.')
        else:
            return print('p value = ',t[1],', deal the daily return by first order difference.')
